...
data class SelcukFlixSearchResponse(
    @JsonProperty("response") val response: String?
)

data class SelcukFlixSearchResultItem(
...
data class SelcukFlixSearchData(
    @JsonProperty("state") val state: Boolean?,
    @JsonProperty("result") val result: List<SelcukFlixSearchResultItem>?
)

data class SelcukFlixSeriesData(
    @JsonProperty("RelatedResults") val relatedResults: SelcukFlixRelatedResults?
)
data class SelcukFlixRelatedResults(
    @JsonProperty("getSerieSeasonAndEpisodes") val getSerieSeasonAndEpisodes: SelcukFlixSerieSeasonAndEpisodes?
)
data class SelcukFlixSerieSeasonAndEpisodes(
    @JsonProperty("result") val seasons: List<SelcukFlixSeason>?
)
data class SelcukFlixSeason(
    @JsonProperty("season_no") val season_no: Int?,
    @JsonProperty("episodes") val episodes: List<SelcukFlixEpisode>?
)
data class SelcukFlixEpisode(
    @JsonProperty("episode_no") val episode_no: Int?,
    @JsonProperty("used_slug") val used_slug: String?
)

class SelcukFlixExtractor : SiteExtractor {
    override val name = "SelcukFlix"
    private val mainUrl = "https://selcukflix.co"
    ...
    override suspend fun extract(
        ...
        try {
            ...
            val jsonResponse = AppUtils.tryParseJson<SelcukFlixSearchResponse>(searchRes)
            val encryptedBlob = jsonResponse?.response ?: return

            val decryptedJson = decryptSelcukFlixResponse(encryptedBlob) ?: return
            ...
            val searchData = AppUtils.tryParseJson<SelcukFlixSearchData>(fixedJson)
            ...
            var epUrl = "$mainUrl/$matchedSlug"
            
            if (season != null && episode != null) {
                // Fetch series page to find correct episode slug
                val seriesHtml = app.get("$mainUrl/dizi/$bareSlug", interceptor = interceptor).text
                val seriesSecureDataMatch = Regex("""\"secureData\":\"([^\"]+)\"""").find(seriesHtml)
                val seriesSecureData = seriesSecureDataMatch?.groupValues?.get(1)
                
                if (seriesSecureData != null) {
                    val decodedSeriesDataRaw = decryptSelcukFlixResponse(seriesSecureData)
                    if (decodedSeriesDataRaw != null) {
                        val decodedSeriesData = if (!decodedSeriesDataRaw.startsWith("{") && decodedSeriesDataRaw.contains("\"essage\"")) {
                            "{m\"$decodedSeriesDataRaw"
                        } else {
                            decodedSeriesDataRaw
                        }
                        try {
                            val seriesDataNode = AppUtils.tryParseJson<SelcukFlixSeriesData>(decodedSeriesData)
                            val seasonsNode = seriesDataNode?.relatedResults?.getSerieSeasonAndEpisodes?.seasons
                            
                            if (seasonsNode != null) {
                                for (sNode in seasonsNode) {
                                    if (sNode.season_no == season) {
                                        val epsNode = sNode.episodes
                                        if (epsNode != null) {
                                            for (epNode in epsNode) {
                                                if (epNode.episode_no == episode) {
                                                    val epSlug = epNode.used_slug?.trim('/')
                                                    if (epSlug != null) {
                                                        epUrl = "$mainUrl/$epSlug"
                                                    }
                                                    break
                                                }
                                            }
                            ...
                            Log.e("plt-stream", "$name JSON parsing error for series page: ${e.message}")
                        }
                    }
                }
            }
            ...
            val decodedData = decryptSelcukFlixResponse(secureData) ?: return
    ...
    private fun decryptSelcukFlixResponse(response: String): String? {
        try {
            ...
            Log.e("plt-stream", "SelcukFlix Decryption error: ${e.message}")
            return null
        }
    }
}
